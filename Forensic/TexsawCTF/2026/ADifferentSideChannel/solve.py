import numpy as np
from Crypto.Cipher import AES

SBOX = [
    
]

HW = [bin(i).count('1') for i in range(256)]

def calculate_correlation(matrix1, matrix2):
    # matrix1: (N, K), matrix2: (N, M) -> result: (K, M)
    m1_c = matrix1 - matrix1.mean(axis=0)
    m2_c = matrix2 - matrix2.mean(axis=0)
    num = np.dot(m1_c.T, m2_c)
    den1 = np.sqrt(np.sum(m1_c**2, axis=0))
    den2 = np.sqrt(np.sum(m2_c**2, axis=0))
    den = np.outer(den1, den2)
    return num / (den + 1e-15)

def solve():
    plaintexts = np.load('plaintexts.npy')
    traces = np.load('traces.npy')
    N_TRACES = plaintexts.shape[0]

    recovered_key = []
    for i in range(16):
        hypotheses = np.zeros((N_TRACES, 256))
        for k in range(256):
            hypotheses[:, k] = [HW[SBOX[p ^ k]] for p in plaintexts[:, i]]
        
        corrs = calculate_correlation(hypotheses, traces)
        max_corrs = np.max(np.abs(corrs), axis=1)
        best_k = np.argmax(max_corrs)
        recovered_key.append(best_k)
    
    key = bytes(recovered_key)
    print(f"Recovered Key: {key.hex()}")

    with open('encrypted_flag.bin', 'rb') as f:
        ciphertext = f.read()

    # The flag starts at block 2 in CBC mode with NULL IV
    cipher = AES.new(key, AES.MODE_CBC, iv=b'\x00'*16)
    decrypted = cipher.decrypt(ciphertext)
    
    try:
        start = decrypted.find(b"texsaw{")
        end = decrypted.find(b"}", start) + 1
        flag = decrypted[start:end].decode()
        print(f"Flag: {flag}")
    except Exception:
        print(f"Could not find flag in decrypted data: {decrypted}")

if __name__ == "__main__":
    solve()
